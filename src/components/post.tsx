...
export const Post = (props: PostCard) => {
  return (
		<div key={props.id} className="block h-96">
			<div className="h-full w-full overflow-hidden bg-slate-100 wrapper rounded inline-block transform transition duration-500 hover:scale-110">
				<div className="relative flex justify-center items-end isolate h-full w-full">
					...
					<div className="flex items-center justify-between absolute top-0 w-full p-2">
						...
						<Link
							className="hover:bg-slate-400 hover:rounded-full hover:text-slate-100 mr-2"
							href={{
								pathname: '/post/[id]', 
								query: { 
								id: props.id
							...
							<svg
								...
								className="h-5 w-5 block lg:inline-block m-1 text-slate-400 float-right hover:text-slate-100"
								fill="currentColor"
								viewBox="0 0 24 24">
